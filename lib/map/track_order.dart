...
class _TrackOrderState extends State<TrackOrder> {
 ...
 Future<void> drawPolyLineFromOriginToDestination() async{
     ...
     var destinationLatlag=LatLng(destinationPosition!.locationLatitude!, destinationPosition!.locationLongitude!);
     
     showDialog(context: context, builder: (BuildContext context)=> ProgressDialog(message: "Please wait...",),);

     var directionDetailsInfo= await Methods.obtainOriginToDestinationDirectionDetails(originLatlng, destinationLatlag);
     setState(() {
   ...
   if(originLatlng.latitude>destinationLatlag.latitude && originLatlng.longitude>destinationLatlag.longitude){
     boundsLatlng =LatLngBounds(southwest: destinationLatlag,northeast: originLatlng);

   }else if(originLatlng.longitude>destinationLatlag.longitude){
     boundsLatlng=LatLngBounds(southwest: LatLng(originLatlng.latitude,destinationLatlag.longitude), northeast: LatLng(destinationLatlag.latitude,originLatlng.longitude),);
   }else if(originLatlng.latitude> destinationLatlag.latitude){
     boundsLatlng=LatLngBounds(southwest: LatLng(destinationLatlag.latitude,originLatlng.longitude), northeast: LatLng(originLatlng.latitude,destinationLatlag.longitude),);
   }else {
     boundsLatlng=LatLngBounds(southwest: originLatlng, northeast: destinationLatlag);
   }


   newGoogleMapController!.animateCamera(CameraUpdate.newLatLngBounds(boundsLatlng, 100));

   Marker originMarker =Marker(markerId: MarkerId("OriginId"),
   infoWindow: InfoWindow(title: originPosition.locationName,snippet: "Origin"),
     position: originLatlng,
     icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueGreen),
   );

     Marker destinationMarker =Marker(markerId: MarkerId("destinationId"),
